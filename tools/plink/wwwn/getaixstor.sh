...
if [ "${stypeemc}" != "" ] ;then
    ...
    do
        disksize=`getconf DISK_SIZE /dev/$i`
        echo ${disksize}
    done | awk '{sum += $1};END {print sum}' >>aixemcfile
    stotal0=`cat aixemcfile`
    stotal=`echo "scale=0;${stotal0}/1024"|bc`
    ...
    ####storage used hba
    v0=`powermt display hba_mode|grep Enabled|awk '{print $2"|"$3"|"$7}'`
    echo "-------------------"
    echo "storage used hba"
    echo "-------------------"
    echo "${v0}"
    ...
    echo "storage used ChaPort"
    echo "-----------------------"
    scha=`powermt display port_mode|grep Enabled|awk '{print $1"|"$2"|"$3"|"$4}'`
    echo "${scha}"
    ...
    echo "map;app configured;app free"
    echo "------------------------------"
    semctype=`powermt display dev=all|grep "Symmetrix ID"|uniq|awk '{print $1}'`
    semcsn=`powermt display dev=all|grep "Symmetrix ID"|uniq|awk -F"=" '{print $2}'`
    ####Oracle version
    dbver=`su - oracle -c ". ./.profile;sqlplus -H|grep "Release""|awk '{print $3}'|grep "10.2.0"`
    dbasm=`ps -ef|grep pmon|grep -v grep|awk '{print $9}'|grep asm`
    dbasm0=`echo ${dbasm}|awk -F"_" '{print $3}'`
    if [ "${dbver}" != "" ] ;then
        ####Release 10.2.0
        ...
        ####Release 11.2.0
        dbver=`su - oracle -c ". ./.profile;sqlplus -H|grep "Release""|awk '{print $3}'|grep "11.2.0"`
        dbasm=`ps -ef|grep pmon|grep -v grep|awk '{print $9}'|grep asm`
        if [ "${dbasm}" != "" ] ;then
            ####hdisk asm administration
            storused=`su - grid -c ". ./.profile;asmcmd -p lsdg"|grep MOU|awk '{print $7}'|awk '{sum += $1};END {print sum}'`
            storfree=`su - grid -c ". ./.profile;asmcmd -p lsdg"|grep MOU|awk '{print $8}'|awk '{sum += $1};END {print sum}'`
            storused1=`echo "scale=0;${storused}/1024"|bc`
            storfree1=`echo "scale=0;${storfree}/1024"|bc`
            echo ""EMC "${semctype}
            ${semcsn}
