...
# Give your file path
filepath = './PUC_AWS_Resources_' + date_fmt + '.csv'
# Give your filename
filename = 'PUC_AWS_Resources_' + date_fmt + '.csv'
csv_file = open(filepath, 'w+')
# Give your owner ID
account_ids = '484879136155'
...
for region in regions:
    ...
    if instanceslist > 0:
        ...
        csv_file.write("%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n" % ('InstanceID', 'Instance_State',
                                                                           'InstanceName', 'Instance_Type', 'LaunchTime', 'Instance_Placement', 'VPC_Id', 'Contact', 'Environment', 'Project', 'Creator', 'Tier', 'Private IP Address', 'Public IP Address', 'Security Group'))
        csv_file.flush()
    ...
    for instance in instances:
        ...
        if state == 'running':
            ...
            csv_file.write("%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n" % (instanceid, state, Instancename, instancetype, launchtime, Placement, VpcId, instance_contact,
                                                                               instance_environment, instance_project, instance_creator, instance_tier, instance_private_ip_address, instance_public_ip_address, instance_security_group))
            csv_file.flush()
...
eccon = boto3.client('elasticache', region_name=reg)
