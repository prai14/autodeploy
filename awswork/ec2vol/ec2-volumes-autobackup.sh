...
create_EBS_Snapshot_Tags() {
  ...
  volume_id=\`${ebs_selected}\`
  i_id=$(aws ec2 describe-volumes  --query 'Volumes[?VolumeId == '${volume_id}'].Attachments[].InstanceId' --output text)
  snapshot_tags="$snapshot_tags Key=InstanceId,Value=$i_id"
  echo "INSTANCE $i_id"
  instance_id=\`$i_id\`
  instance_name=$(aws ec2 describe-instances  --query 'Reservations[].Instances[?InstanceId=='${instance_id}'].Tags[][?Key==`Name`].Value[]' --output text)
  echo "INSTANCE NAME: $instance_name" 
  snapshot_tags="$snapshot_tags Key=InstanceName,Value=$instance_name"
...
if [[ -z $region ]]; then
  #if the environment variable $EC2_REGION is not set set to cn-north-1
